...
from Algorithm2Domain_AdaTime.trainers.sweep import Trainer as SweepTrainer
from Algorithm2Domain_AdaTime.trainers.train import Trainer as TrainTrainer
from Algorithm2Domain_AdaTime.configs.data_model_configs import *
from Algorithm2Domain_AdaTime.configs.sweep_params import sweep_alg_hparams
...
def training_strategy():
    ...
    with st.expander("Training Strategy Configuration", expanded=True):
        ...
        if st.session_state.algorithm != "Select Algorithm":
            ...
            if st.session_state.hyperparameter_tuning_config["hyperparameter_tuning"] == "Hyperparameter Tuning":
                ...
                if st.session_state.algorithm in sweep_alg_hparams:
                    algo_hparams = sweep_alg_hparams[st.session_state.algorithm]
        ...
        if st.button("Validate Training Strategy", use_container_width=True):
            # Check if each field is filled
            # Algorithm
            if not st.session_state.algorithm or st.session_state.algorithm == "Select Algorithm":
                st.error("Please select an Algorithm!")
            # Number of Epochs
            elif st.session_state.number_of_epochs < 1:
                st.error("Number of Epochs must be at least 1!")
            # Batch Size
            elif st.session_state.batch_size < 1:
                st.error("Batch Size must be at least 1!")
            # Weight Decay
            elif not (0 <= st.session_state.weight_decay <= 1):
                st.error("Weight Decay must be between 0 and 1!")
            # If all fields are valid
            else:
                alg_params = {}
                if st.session_state.algorithm != "Select Algorithm":
                    for key in algo_specific_keys.get(st.session_state.algorithm, []):
                        if key not in common_keys:
                            alg_params[key] = st.session_state.get(key)
                ...
                training_strategy_config = {
                    ...
                    "num_epochs": st.session_state.number_of_epochs,
                    "batch_size": st.session_state.batch_size,
                    "weight_decay": st.session_state.weight_decay,
                    "step_size": st.session_state.step_size,
                    "lr_decay": st.session_state.lr_decay,
                    **alg_params,
                }

                if st.session_state.hyperparameter_tuning_config["hyperparameter_tuning"] == "Hyperparameter Tuning":
                    # Add hyperparameters to the training strategy config
                    training_strategy_config[st.session_state.algorithm] = hyperparams_dict
                
                # if st.session_state.hyperparameter_tuning_config["hyperparameter_tuning"] == "Hyperparameter Tuning":
                #     training_strategy_config = {
                #         "algorithm": st.session_state.algorithm,
                #         "num_epochs": st.session_state.hyperparameter_tuning_config["num_epochs"],
                #         "batch_size": st.session_state.hyperparameter_tuning_config["batch_size"],
                #         "weight_decay": st.session_state.hyperparameter_tuning_config["weight_decay"],
                #         "step_size": st.session_state.hyperparameter_tuning_config["step_size"],
                #         "gamma": st.session_state.hyperparameter_tuning_config["gamma"],
                #         "optimizer": st.session_state.hyperparameter_tuning_config["optimizer"],
                #         "lr_decay": st.session_state.lr_decay,
                #         "learning_rate": st.session_state.hyperparameter_tuning_config["learning_rate"],
                #         "disc_lr": st.session_state.hyperparameter_tuning_config["disc_lr"],
                #         **alg_params,
                #         st.session_state.algorithm: hyperparams_dict
                #     }
...
def results(summary_best):
    ...
    logs_folder_path = f"ADATime/experiments_logs/{st.session_state.evaluation_setup['dataset_choice']}/{st.session_state.training_strategy['algorithm']}_{st.session_state.evaluation_setup['experiment_name']}"
...
def run_experiment_ADATime():
    ...
    args_for_train = argparse.Namespace(
        ...
        exp_name=st.session_state.evaluation_setup["experiment_name"],
        da_method=st.session_state.training_strategy["algorithm"],
        data_path="ADATIME_data",
        dataset=st.session_state.evaluation_setup["dataset_choice"],
        backbone=st.session_state.backbone_model_config["backbone_model"],
        num_runs=st.session_state.evaluation_setup["number_of_runs"],
        device=device,
    )
    ...
    dataset_class = get_dataset_class(st.session_state.evaluation_setup["dataset_choice"])
    dataset_config = dataset_class()

    # Common dataset configuration parameters
    dataset_config.scenarios = st.session_state.evaluation_setup["scenarios"]
    dataset_config.stride = st.session_state.backbone_model_config["stride"]
    dataset_config.kernel_size = st.session_state.backbone_model_config["kernel_size"]
    dataset_config.final_out_channels = st.session_state.backbone_model_config["final_outcome_channels"]
    dataset_config.features_len = st.session_state.backbone_model_config["features_length"]
    dataset_config.normalization = True if st.session_state.preprocessing["normalization"] == "Standard Normalization" else False
    dataset_config.data_augmentation_configs = st.session_state.data_augmentation

    # For Discriminator-based algorithms, set the discriminator parameters
    if st.session_state.training_strategy["algorithm"] == ["DANN", "CDAN", "DIRT", "CoDATS", "AdvSKM"]:
        dataset_config.disc_hid_dim = st.session_state.training_strategy["disc_hid_dim"]
        dataset_config.hidden_dim = st.session_state.training_strategy["hidden_dim"]
        dataset_config.DSKN_disc_hid = st.session_state.training_strategy["DSKN_disc_hid"]

    # Model-specific dataset configuration updates
    backbone = st.session_state.backbone_model_config["backbone_model"]
    if backbone == "CNN":
        # For CNN, update mid_channels if available
        if "mid_channels" in st.session_state.backbone_model_config:
            dataset_config.mid_channels = st.session_state.backbone_model_config["mid_channels"]
    elif backbone == "LTCN":
        # For LTCN, update hidden_size and ode_solver_unfolds
        dataset_config.hidden_size = st.session_state.backbone_model_config["ltcn_hidden_size"]
        dataset_config.ode_solver_unfolds = st.session_state.backbone_model_config["ltcn_ode_solver_unfolds"]
    elif backbone == "TCN":
        # For TCN, convert tcn_layers from string to list of integers and update TCN parameters
        tcn_layers_str = st.session_state.backbone_model_config["tcn_layers"]
        try:
            tcn_layers_list = [int(x.strip()) for x in tcn_layers_str.split(",")]
        except Exception as e:
            tcn_layers_list = [75, 150]
        dataset_config.tcn_layers = tcn_layers_list
        dataset_config.tcn_final_out_channels = st.session_state.backbone_model_config["tcn_final_out_channels"]
        dataset_config.tcn_kernel_size = st.session_state.backbone_model_config["tcn_kernel_size"]
        dataset_config.tcn_dropout = st.session_state.backbone_model_config["tcn_dropout"]

    # Run training if phase is train
    if args_for_train.phase == 'train':
        trainer = TrainTrainer(args_for_train)
        trainer.fit(hparams=st.session_state.training_strategy, dataset_configs=dataset_config)

    # Run testing if phase is test
    if args_for_test.phase == 'test':
        trainer = TrainTrainer(args_for_test)
        summary_best = trainer.test(hparams=st.session_state.training_strategy, dataset_configs=dataset_config)
        return summary_best
...
def run_sweep_ADATime():
    ...
    with st.expander("Sweep", expanded=True):
        ...
        args_for_sweep = argparse.Namespace(
            ...
            num_runs=st.session_state.evaluation_setup["number_of_runs"],
            device=device,
            exp_name=st.session_state.hyperparameter_tuning_config["sweep_project_name"],
            num_sweeps=st.session_state.hyperparameter_tuning_config["num_sweeps"],
            sweep_project_wandb="TEST",
            wandb_entity="",
            hp_search_strategy="random",
            metric_to_minimize=st.session_state.hyperparameter_tuning_config["target_risk_score"],
        ...
        if backbone == "CNN":
            ...
            try:
                ...
                tcn_layers_list = [32, 64]
            dataset_config.tcn_layers = tcn_layers_list
            dataset_config.tcn_kernel_size = st.session_state.backbone_model_config["tcn_kernel_size"]
            dataset_config.tcn_dropout = st.session_state.backbone_model_config["tcn_dropout"]

        sweep_hparams = {
            st.session_state.training_strategy["algorithm"]: st.session_state.training_strategy[st.session_state.training_strategy["algorithm"]]
        }

        trainer = SweepTrainer(args_for_sweep)

        results_list = trainer.sweep(dataset_configs=dataset_config, sweep_hparams=sweep_hparams, hparams=st.session_state.training_strategy)
        ...
        sweep_id = st.session_state.hyperparameter_tuning_config["sweep_project_wandb"]
        wandb_url = f"https://wandb.ai/{username}/{sweep_id}"
